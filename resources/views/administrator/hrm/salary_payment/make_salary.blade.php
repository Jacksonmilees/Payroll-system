  ...
  <section class="content">
            ...
            <div class="col-md-12 table-responsive">
             ...
             <table class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td>{{ format_employee_name($user['name'], $user['name_prefix'] ?? null) }}</td>
                </tr>
                <tr>
    ...
    <div class="col-md-3">
        ...
        <div class="box-body">
          <form action="{{ url('/hrm/salary-payments/store') }}" method="post">
            ...
            <div class="form-group">
              ...
              <input type="date" name="payment_date" class="form-control" id="payment_date" value="{{ old('payment_date', $default_payment_date) }}">
            </div>
            ...
            <div class="form-group">
              <label>{{ __('Worked Days (Present)') }}</label>
              <input type="text" class="form-control" value="{{ $attendanceSummary['work_days'] ?? 0 }}" readonly>
            </div>

            <div class="form-group">
              <label>{{ __('Recorded Hours') }}</label>
              <input type="text" class="form-control" value="{{ number_format($attendanceSummary['work_hours'] ?? 0, 2) }}" readonly>
            </div>
      ...
      <div class="box box-info">
        <div class="box-header with-border">
          <h3 class="box-title">{{ __('Work Summary') }}</h3>
        </div>
        <div class="box-body">
          <p><strong>{{ __('Pay Structure:') }}</strong> {{ ucfirst($pay_basis ?? 'monthly') }}</p>
          <p><strong>{{ __('Present Days:') }}</strong> {{ $attendanceSummary['work_days'] ?? 0 }}</p>
          <p><strong>{{ __('Absent Days:') }}</strong> {{ $attendanceSummary['absent_days'] ?? 0 }}</p>
          <p><strong>{{ __('Leave Days:') }}</strong> {{ $attendanceSummary['leave_days'] ?? 0 }}</p>
          <p><strong>{{ __('Work Hours:') }}</strong> {{ number_format($attendanceSummary['work_hours'] ?? 0, 2) }}</p>
        </div>
      </div>
    </div>
    <!-- /.end.col -->
      ...
      <div class="box box-primary">
        ...
        <div class="box-body">
          <table class="table table-bordered">
            ...
            <tr>
              <td>{{ $sl++ }}</td>
              <td>{{ $allowance['deduction_name'] }}</td>
              <td>
                @php($credits += $allowance['deduction_amount'])
                <input type="hidden" name="item_name[]" value="{{ $allowance['deduction_name'] }}">
                <input type="hidden" name="amount[]" value="{{ $allowance['deduction_amount'] }}">
                <input type="hidden" name="status[]" value="credits">
              </td>
              <td>
                {{ $allowance['deduction_amount'] }}
              </td>
            </tr>
            @endforeach
            ...
            @if(!empty($salary['nhif']))
            <tr>
              <td>{{ $sl++ }}</td>
              <td>{{ __('NHIF') }}</td>
              <td>
                @php($debits += ($salary['basic_salary']* 2.75 / 100))
                -{{($salary['basic_salary']* 2.75 / 100)}}
                <input type="hidden" name="item_name[]" value="NHIF">
                <input type="hidden" name="amount[]" value="{{ ($salary['basic_salary']* 2.75 / 100)}}">
                <input type="hidden" name="status[]" value="debits">
              </td>
              <td></td>
            </tr>
            ...
            @if(!empty($salary['nssf']))
            <tr>
              <td>{{ $sl++ }}</td>
              <td>{{ __('NSSF') }}</td>
              <td>
                @php($debits += ($salary['basic_salary'] <= 36000 ? $salary['basic_salary'] * 6 / 100 : 2160))
                -{{($salary['basic_salary'] <= 36000 ? $salary['basic_salary'] * 6 / 100 : 2160)}}
                <input type="hidden" name="item_name[]" value="NSSF">
                <input type="hidden" name="amount[]" value="{{ ($salary['basic_salary'] <= 36000 ? $salary['basic_salary'] * 6 / 100 : 2160) }}">
                <input type="hidden" name="status[]" value="debits">
              </td>
              <td></td>
            </tr>
