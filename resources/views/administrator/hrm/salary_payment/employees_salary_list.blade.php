                ...
                <div class="form-group">
                  <div class="col-sm-offset-3 col-sm-10">
                    ...
                    <button type="button" class="btn btn-secondary btn-flat" onclick="openLinks()">
                      <i class="fa fa-print"></i> {{ __('Print All') }}
                    </button>
                  </div>
                </div>
                <!-- /.end group -->
        ...
        <div class="box box-primary">
          ...
          <div class="box-body">
            ...
            <div class="col-md-12">
              ...
              <tr>
                <td>{{ $sl++ }}</td>
                <td>{{ format_employee_name($employee['name'], $employee['name_prefix'] ?? null) }}</td>
               ...
               @if($deduction['deduction_name'] == 'advance')
                ...
                @endif
                @if($deduction['deduction_name'] == 'loan')
                ...
                <td> {{  number_format($advance + $loana, 2, '.', ',') }}</td>
                <td> {{  number_format($houseleavy, 2, '.', ',') }}</td>
                <td>{{ number_format($employee['paye'] + $employee['nhif'] + $employee['nssf'] + $advance + $loana + $otherd + ($employee['basic_salary'] * 1.5)/100, 2, '.', ',') }}</td>
             
                <td>{{ number_format($credits - ($employee['paye'] + $employee['nhif'] + $employee['nssf'] + $advance + $loana + $otherd + ($employee['basic_salary'] * 1.5)/100), 2, '.', ',') }}</td>
                ...
                <td>
                  @if(!empty($salary_payments))
                    ...
                    @endforeach
                    @if($status == 1)
                      <a href="{{ url('hrm/salary-payments/manage-salary/' . $employee['user_id'] . '/' . $salary_month) }}" target="_blank"><p class="text-success">{{ __('Paid') }}</p></a>
                      <a href="{{ url('/hrm/salary-payments/pdf/'.$employee['user_id'].'/'.$salary_month) }}" target="_blank" class="d-none links"></a>
                    @else
