...
Route::group(['middleware' => 'auth'], function () {
	...
	Route::post('/my/attendance/view', 'AttendanceController@myAttendanceCSV');
	Route::post('/my/attendance/store', 'AttendanceController@myAttendanceCSVStore');
	Route::post('import-excel','ImportExcel\ImportExcelController@import');

	//Machine Activation//
    Route::get('/machine/activation', 'AttendanceController@machineActivation');
    Route::post('/machine/activation/update', 'AttendanceController@machineActivationUpdate');
    Route::get('/machine/manual/setting', 'AttendanceController@machineManualSetting');
    Route::get('/machine/manual/setting', 'AttendanceController@machineManualSetting');
    Route::get('/machine/attendance/manage', 'AttendanceController@attendanceManage');
    Route::get('/machine/attendance/report', 'AttendanceController@attendanceReport');
    Route::post('/machine/attendance/report/details', 'AttendanceController@attendanceReportDetails');
    Route::get('/machine/attendance/edit/{id}', 'AttendanceController@attendanceEdit');
    Route::post('/machine/attendance/update', 'AttendanceController@attendanceUpdate');
	...
	Route::get('/hrm/leave-reports/leave-count', 'LeaveAppController@leaveCount')->name('leave.count');

	//Noc and Ex Certificate Application//
	...
	Route::redirect('/people/clients','/people/employees');
	Route::redirect('/setting/client-types','/people/employees');
	// Customer Section //
	...
	Route::get('/people/employees-report', 'EmplController@report');
	Route::get('/people/employees-report-pdf', 'EmplController@reportPdf');

	// Reports
	Route::get('/hrm/payroll-report-detail', 'EmplController@reportDetail');
	Route::get('/hrm/nssf-report', 'EmplController@nssfReport');
	Route::get('/hrm/nhif-report', 'EmplController@nhifReport');
	Route::get('/hrm/paye-report', 'EmplController@payeReport');
    Route::get('/hrm/p9-report', 'EmplController@pNineReport');
	Route::get('/hrm/reports/salary', 'ReportController@salary')->name('reports.salary');
	...
	Route::get('/hrm/expence/payslip/{id}', 'ExpenceManagementController@ExpensePayslip');
	...
	// Deduction Section //
	Route::redirect('/hrm/deductions','/hrm/deduction');
	Route::redirect('/hrm/allowances','/hrm/allowance');
	Route::get('/hrm/allowance', 'DeductionController@allowance_index');
	Route::get('/hrm/deduction', 'DeductionController@index');
	Route::get('/hrm/{type}/create', 'DeductionController@create');
	Route::post('/hrm/{type}/store', 'DeductionController@store');
	Route::get('/hrm/{type}/details/{id}', 'DeductionController@show');
	Route::get('/hrm/{type}/edit/{id}', 'DeductionController@edit');
	Route::post('/hrm/{type}/update/{id}', 'DeductionController@update');
	Route::get('/hrm/{type}/delete/{id}', 'DeductionController@destroy');
	
	Route::post('/setting/update', 'HomeController@logoUpdate')->name('logo.update');
});
...
// 		\Illuminate\Support\Facades\Artisan::call('route:clear');
		\Illuminate\Support\Facades\Artisan::call('vendor:publish',['--all' => true]);
		return 'Cleared!';		
	});
// Route::get('/clear', 'HomeController@cache_clear');
